...
//if  string you write a method on a string you need to store it in a variable cuz it won't change the original it creates a new string while stringbuilder works on same string it we don't need to store it in a variable it changes the original string
...
//a==b,gives true for if the ref variale both a and b are poiting to the same object else it gives false if the ref variable are pointing uniquely even if the object is the same.so equal equal checks memory addresses i.e. references and eqauls to explicitly checks values only.
...
// length()    //say name.length() //parnthesis should be there in case of string methods   //return length of string
// substring(int start,int end)    //returns substring from start to end-1
// toString()     //converts it into a new different string from stringbuilder
...
//         // if(str==st){
//         //     return true;      
//         // }                        //You can't use == for string and stringbuilder as both are different types
//         // else{
//         //     return false;
//         // }
